...
export default function Collapse({ name, type }: { name: string; type: number }) {
	...
	return (
		<div ref={wrapperRef} className="w-full border-b border-gray-700 pt-8">
			...
			<div ref={contentRef} className={"flex flex-wrap max-h-0 pt-8 pl-10"}>
				{type === 1 && (
					<p className="text-base xl:text-lg font-light text-gray-300">
						...
						launch contracts from a centralised server, making it easier for hackers to launch a malicious contract from
						the same address. SAFU Launcher is a smart contract that is immutable and open source, hence any contract
						launched by the SAFU Launcher smart contract address is deemed safu. SAFU Launcher can also be integrated
						with any dApp or smart contract to safely launch a project.
						<br />
						<br />
						...
						liquidity tokens. SAFU Launcher's native token vesting ensures a fair and transparent allocation process and
						supports staking pools for the launched ERC20 tokens. These features are in-built and available to users for
						free.
						<br />
						<br />
				...
				{type === 2 && (
					<p className="text-base xl:text-lg font-light text-gray-300">
						...
						vesting features are free and inbuilt. Every token launched on the platform can launch its own staking pool
						with rewards in $ETH. SAFU Launcher prevents sniper attacks by allowing projects to set high initial taxes
						for a short duration on launch, and ensure that it drops below an acceptable tax percentage of 6 percent
						automatically so a regular user is not penalised.
						<br />
						<br />
						<a
				...
				{type === 4 && (
					<p className="text-base xl:text-lg font-light text-gray-300">
						Projects can launch projects for free on SAFU Launcher. Unlike existing IDO platforms, SAFU Launcher offers
						features like liquidity locking and token vesting for free.
					</p>
				)}
				{type === 5 && (
					<p className="text-base xl:text-lg font-light text-gray-300">
						SAFU Launcher is compatible across multiple chains. SAFU supports Base, BSC, Polygon, Optimism, Arbitrium,
						Avalanche and Blast chains at present and will expand the support to other EVMs (Ehtereum Virtual Machines)
						soon.
					</p>
				)}
			</div>
